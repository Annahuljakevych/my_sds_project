...
- "static-layout": a directory representing a Python project that contains a "static"
  folder. Everything under the "static" folder is meant to be included in the
...
- "flat-layout": a Python project that does not use "static-layout" but instead
...
class ConfigDiscovery:
    ...
    def _analyse_explicit_layout(self) -> bool:
        ...
        package_dir.pop("", None)  # This falls under the "static-layout" umbrella
    ...
    def _analyse_src_layout(self) -> bool:
        """Try to find all packages or modules under the ``static`` directory
        (or anything pointed by ``package_dir[""]``).

        The "static-layout" is relatively safe for automatic discovery.
        We assume that everything within is meant to be included in the
        distribution.

        If ``package_dir[""]`` is not given, but the ``static`` directory exists,
        this function will set ``package_dir[""] = "static"``.
        """
        package_dir = self._package_dir
        src_dir = os.path.join(self._root_dir, package_dir.get("", "static"))
        if not os.path.isdir(src_dir):
            return False

        log.debug(f"`static-layout` detected -- analysing {src_dir}")
